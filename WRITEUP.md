...
`run_polybot.ps1` is the daily loop: starts at 12:01 AM ET, stops trading at 11:30 PM ET, runs the pipeline at 11:45 PM ET, commits + pushes, restarts at the next midnight ET. The outer `while ($true)` survives auth errors but won't retry the same day — fix auth before midnight.

## 16. Invariants (what doesn't drift)
...
- **Daily rollup** at 12:05 AM ET bundles per-trade JSON into `rollup_YYYY-MM-DD.json`.
