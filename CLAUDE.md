...
**Sections 1–9 are PERMANENTLY FROZEN** — the trading logic (model, gates, sizing, ordering, exits, flips, resolution, loss handling). Code and spec are reconciled: no structural change and no hand-edit to these sections or the code implementing them — ever. The only thing that moves is the numeric knobs in §12, which the nightly pipeline (§11) tunes *inside* these fixed structures; it can never restructure §1–9.
...
Part A describes the trading mechanism. The structural decisions (Student-t over Gaussian, polarity-split L4, regime-damped L5, closed L6 library, redundancy-discounted L3+L3b+L3e flow combine, isotonic-only calibration, the entry-gate set, the sizing pipeline, the exit-branch order, the flip-premium formula, Chainlink-only resolution, the loss-handling stack) are locked. The pipeline tunes only the **numeric values** within them (§12).
...
- **7-day holdout** — last 7 days excluded from all folds AND the evolver's context. The calibrator's fit pool sits in a **separate** 7-day window immediately before the holdout (days `[HOLDOUT_DAYS, HOLDOUT_DAYS + _CAL_WINDOW_DAYS]` back), so the holdout-confirmation gate evaluates candidates on trades the calibrator never saw.
...
`IsotonicCalibrator.fit` operates on its own 7-day pool immediately before the holdout (days `[HOLDOUT_DAYS, HOLDOUT_DAYS + _CAL_WINDOW_DAYS]` back), disjoint from the holdout so the confirmation gate is genuinely OOS for the calibrator. Pool must hold **≥125 trades**, split 60/40 into `cal_train` / `cal_val`. `fit` is called with **`min_samples=75`** (overriding the class default 150), so `cal_train` must be ≥75 to fit, and the Kelly-Sharpe gate ((iii) in stage 3) needs **≥50** `cal_val` trades. See §2 for the per-fit bootstrap-CI gate.
