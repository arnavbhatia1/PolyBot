...
def test_gate_sigma_freezes_then_graduates_on_toxic_oos(tmp_path):
    ...
    _seed(reg_path)
    lo = _ctx(regime_buckets={"atr_regime": "LO", "session": "DAY", "burst": "HOT"})
    # 9 OOS days strictly after discovery, day-gains ~+11..19¢ (vetoing wins):
    # first 4 freeze σ, the rest score and cross the accept boundary.
    fills = [(f"2026-07-{d:02d}", -(0.10 + 0.01 * (d % 5)), 1.0, "Up", 0.88, lo)
