...
class TestRegimeDetector:
    def test_trending_up_detected(self):
        ...
        state = det.classify(closes=closes, atr=25.0, atr_history=[20, 22, 24, 25, 26])
    ...
    def test_mean_reverting_detected(self):
        ...
        det = RegimeDetector(lookback=20)
        state = det.classify(closes=closes, atr=25.0, atr_history=[15, 20, 25, 30, 35])
    ...
    def test_volatile_detected(self):
        ...
        state = det.classify(closes=closes, atr=80.0, atr_history=[20, 25, 30, 25, 20])
    ...
    def test_quiet_detected(self):
        ...
        state = det.classify(closes=closes, atr=3.0, atr_history=[20, 25, 30, 25, 20])
        assert state.name == "quiet"
        assert state.skip is True

    def test_default_when_insufficient_data(self):
        det = RegimeDetector(lookback=20)
        state = det.classify(closes=np.array([100.0]), atr=25.0, atr_history=[])
    ...
    def test_default_lookback_is_50(self):
        ...
        state = det.classify(closes=closes, atr=25.0, atr_history=[20, 25, 30])
        assert state.name != "unknown"
