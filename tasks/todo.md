...
## Open (operator call, not now)
- [ ] Microstructure recorder still writes ~3.5MB/day of now-concluded data — keep for future re-runs or disable in main.py?
- [ ] Code changes (scheduler/calibrator/signal_engine + test) are uncommitted — commit when ready (nightly auto-commit only sweeps settings/memory/db).
